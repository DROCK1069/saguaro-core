 ...
 */
import { projectContractValueOrNull } from './projectContract';
...
export function computeHealth(p: AnyProject, nowMs?: number): ProjectHealth {
  ...
  // ---- Budget ----
  const budget = projectContractValueOrNull(p); // canonical revised-first contract value (projectContract.ts) — one rule platform-wide
  ...
  const status: HealthStatus = reds > 0 ? 'red' : yellows > 0 ? 'yellow' : 'green';
