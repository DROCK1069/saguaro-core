...
export const STAGE_IDS: string[] = ROLLOUT_STAGES.map((s) => s.id);
...
export const STAGE_META: Record<string, RolloutStage> = Object.fromEntries(
  ROLLOUT_STAGES.map((s) => [s.id, s]),
);

// Guard for a persisted `rollout_stage` value (e.g. validating a write payload).
export const isStage = (v: unknown): boolean =>
  typeof v === 'string' && STAGE_IDS.includes(v);
