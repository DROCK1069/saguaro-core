...
export const MIN_RETAINAGE_PCT = 0;
export const MAX_RETAINAGE_PCT = 100;
 ...
 */
export const RETAINAGE_HIGH_PCT = 20;

/** Clamp any number into the valid 0–100 retainage band. */
export function clampRetainagePct(pct: number): number {
  if (!Number.isFinite(pct)) return DEFAULT_RETAINAGE_PCT;
  return Math.min(MAX_RETAINAGE_PCT, Math.max(MIN_RETAINAGE_PCT, pct));
}
 ...
 */
export function resolveRetainagePct(
  value?: number | string | null,
  fallback: number = DEFAULT_RETAINAGE_PCT,
): number {
  if (value === null || value === undefined) return clampRetainagePct(fallback);
  if (typeof value === 'string') {
    const trimmed = value.trim();
    if (trimmed === '') return clampRetainagePct(fallback);
 ...
 */
export function retainageCents(amountCents: Cents, pct: number): Cents {
  return percentOf(amountCents, clampRetainagePct(pct));
}

/** Human label for a retainage rate, e.g. 10 → "10%". */
export function retainageLabel(pct: number): string {
  return `${clampRetainagePct(pct)}%`;
}

/** True when the rate is above the "verify this" threshold (see sanity layer). */
export function isRetainageUnusuallyHigh(pct: number): boolean {
  return clampRetainagePct(pct) > RETAINAGE_HIGH_PCT;
}
